function plotEVMandMER(BS_fieldnames, EVM_collection, MER_collection, frameNum, numFrames)
    % plotEVMandMER - 动态绘制每个基站的 EVM 和 MER
    %
    % 输入参数：
    ...
    for i = 1:numBS
        ...
        yyaxis left;
        set(hPlotEVM_Header(i), 'YData', EVM_collection.(current_BS_field).header(1:frameNum));
        set(hPlotEVM_Header(i), 'XData', 1:frameNum);  % 更新横轴
        set(hPlotEVM_Data(i), 'YData', EVM_collection.(current_BS_field).data(1:frameNum));
        set(hPlotEVM_Data(i), 'XData', 1:frameNum);    % 更新横轴
        
        % 更新 MER header 和 data
        yyaxis right;
        set(hPlotMER_Header(i), 'YData', MER_collection.(current_BS_field).header(1:frameNum));
        set(hPlotMER_Header(i), 'XData', 1:frameNum);  % 更新横轴
        set(hPlotMER_Data(i), 'YData', MER_collection.(current_BS_field).data(1:frameNum));
        set(hPlotMER_Data(i), 'XData', 1:frameNum);    % 更新横轴
    end
